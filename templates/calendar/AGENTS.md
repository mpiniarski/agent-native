...
| `find-a-time`                        | `--date` or `--from`/`--to`, `--attendees`, optional `--durationMinutes`, `--timezone`, `--accountEmail`, `--ignoreStart`, `--ignoreEnd`                                                                                                                                                                                                                                                                             | Find shared free slots for the organizer and guests using Google free/busy plus local conflicts        |
...
For multi-person scheduling, use `find-a-time` before creating or moving the event. Pass attendee emails as a comma-separated `--attendees` list, `--durationMinutes` for meeting length, and either `--date <YYYY-MM-DD>` to search that week or explicit `--from`/`--to` bounds. When rescheduling an existing event, pass its current `--ignoreStart` and `--ignoreEnd` so the meeting does not conflict with itself.
...
| "Find a 1-hour slot this week"      | `find-a-time --date <week-start-or-target-day> --durationMinutes 60`                                                  |
| "Find time with Alice next week"    | `search-people --q Alice`, then `find-a-time --date <next-week-date> --attendees alice@example.com`                   |
