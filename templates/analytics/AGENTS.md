...
2. Create or open the target dashboard early, then navigate to it. For new dashboards, save the smallest valid dashboard shell with `update-dashboard` so the user has a durable artifact to watch while you work.
3. For longer dashboard builds, use `manage-progress` to expose visible checkpoints such as source discovery, metric validation, first panel saved, and remaining panels. Do not create a separate streaming or progress system.
4. **Check the `<data-dictionary>` block** in your system prompt for entries that match the user's request.
...
| ------------------------------ | --------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `data-source-status`           | `[--key <provider-or-key>]`             | Show configured data-source credentials and granted workspace connections for `appId=analytics` without revealing values. Accepts aliases like `jira`, `pylon`, `github`, `bigquery`, `hubspot`, `gong`, `slack`.                                                     |
| `github-code`                  | `--mode`, `--repo`, `--query`, `--path` | GitHub repository listing, code search, and file reads. Use for source-code questions, tracking instrumentation discovery, and finding strings such as `trackI`.                                                                                                      |
| `github-prs`                   | `--org`, `--query`                      | GitHub PR & issue search                                                                                                                                                                                                                                              |
...
| `generate-chart`               | `--type`, `--data`                      | Generate a static chart image **for `save-analysis` artifacts only**. Uses PNG when the native renderer is available and SVG fallback on hosted runtimes without native canvas. For in-chat answers, use a live `/chart` embed instead (see "Inline Charts in Chat"). |
...
| "Find all trackI() calls"            | `github-code --mode=search-code --repo=<owner/repo> --query='"trackI"'`, then `github-code --mode=get-file --repo=<owner/repo> --path=<path>` for the relevant files         |
...
- **Saving a `save-analysis` artifact → static image via `generate-chart`.** Only use the static image path when the chart needs to survive outside this app (an emailed report, an analysis artifact a teammate reads later). The action uses PNG when native canvas is available and returns an SVG fallback when hosted runtimes cannot load native canvas. If the user is asking a question, this is the wrong tool.
