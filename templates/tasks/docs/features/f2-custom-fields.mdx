...
**Custom fields** extend [F1 tasks](./f1-tasks.mdx) with user-defined properties — due dates, notes, priority, and anything else the user models. Fields are defined once for the app; each task fills in its own values.

[F4 Inbox](./f4-inbox.mdx) items can preset custom-field values before promotion; tasks carry those values after mark ready.
...
[Search](./f6-search.mdx) excludes custom-field values.
...
Deleting a field definition removes its values on every task. Saved [views](./f5-views.mdx) that filter, sort, or group by that field need to be updated.
