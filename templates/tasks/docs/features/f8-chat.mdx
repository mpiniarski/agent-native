...
- **Work with custom fields** — define fields and set values on tasks ([F2](./f2-custom-fields.mdx)).
...
- **Find things** — [search](./f6-search.mdx) by task title, project name, or saved-view name and open a result.
...
| Area                                           | Agent can…                                                                                   |
| ---------------------------------------------- | -------------------------------------------------------------------------------------------- |
| **[F1 Tasks](./f1-tasks.mdx)**                 | Add, edit, delete tasks; mark complete/incomplete; reorder; bulk complete/delete             |
...
| Area                                 | Agent can…                                                            |
| ------------------------------------ | --------------------------------------------------------------------- |
| **[F4 Inbox](./f4-inbox.mdx)**       | Assign project (hidden until ready)                                   |
...
| **[F6 Search](./f6-search.mdx)**     | Search tasks, projects, and saved views; open a result                |
...
- **Completion** on inbox items is not allowed until **mark ready** ([F4](./f4-inbox.mdx)).

<!-- status: todo -->

- Inbox items with a project assigned do **not** appear on the project page until **mark ready**.
- **Search** matches task titles, project names, and saved-view names — not custom field values.
- **Sort by field** disables manual reorder until sort is **None** again.

### Context-aware

- **`view-screen`** is the first call when the user's visible context matters — which page, selected tasks, active filter, project, view, or search query.
